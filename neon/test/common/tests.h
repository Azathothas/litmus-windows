...
/* Set non-zero by the NEON_TEST_INIT function to emit a single JSON
 * object describing the run on stdout, instead of the normal
 * human-readable output.  Default output is unchanged when zero. */
extern int test_json;

/* Set non-zero by the NEON_TEST_INIT function to send the protocol
 * debug trace to stderr rather than only to debug.log. */
extern int test_verbose;

/* Optional description of what is under test (for litmus, the target
 * URL), included in the JSON output when non-NULL. */
extern const char *test_target;

/* Set by the NEON_TEST_INIT function to the stream that the wire dump
 * is being written to, so that the harness sends the protocol trace
